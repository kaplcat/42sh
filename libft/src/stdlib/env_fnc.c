...
static char *create_env_record(const char *name, const char *value)
{
    size_t  namelen;
    char    *rec;
...
int env_append_record(char **env, const char *name, const char *value)
{
    ...
    *env = create_env_record(name, value);
    if (*env == NULL)
    {
        return (-1);
    }
...
int env_update_record(char **env, const char *name, const char *value)
{
    ...
    if (value_len < ft_strlen(value))
    {
        new_rec = create_env_record(name, value);
        if (new_rec == NULL)
        {
            return (-1);
        }
